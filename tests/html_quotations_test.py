...
from tests.fixtures import OLK_SRC_BODY_SECTION, REPLY_QUOTATIONS_SHARE_BLOCK, REPLY_SEPARATED_BY_HR
...
<blockquote>
  ...
  <div>
    ...
    eq_("<html><head></head><body>Reply</body></html>", RE_WHITESPACE.sub("", quotations.extract_from_html(msg_body)))
...
Reply
    ...
    eq_(RE_WHITESPACE.sub("", reply), RE_WHITESPACE.sub("", quotations.extract_from_html(msg_body)))
...
"""
    ...
    ok_("<html>" in out and "</html>" in out, "Invalid HTML - <html>/</html> tag not present")
...
"""
    eq_(RE_WHITESPACE.sub("", stripped_html), RE_WHITESPACE.sub("", quotations.extract_from_html(msg_body)))
...
"""
    eq_("<html><head></head><body><div>message<br></div></body></html>", RE_WHITESPACE.sub("", quotations.extract_from_html(msg_body)))
...
<body>
  <div>
    ...
    eq_(
        "<html><head></head><body><div>Blah<br><br></div></body></html>", RE_WHITESPACE.sub("", quotations.extract_from_html(msg_body))
    )
...
"""
    eq_("<html><head></head><body>Reply<div><hr></div></body></html>", RE_WHITESPACE.sub("", quotations.extract_from_html(msg_body)))
