...
import pytest  # Added for assert_raises equivalent
...
Roman"""
    assert ("Hey!", "--\nBest regards,\n\nRoman") == bruteforce.extract_signature(
        msg_body
    )
...
Sent from Mailbox for iPhone"""
    assert (("Blah", "Sent from Mailbox for iPhone")) == bruteforce.extract_signature(
        msg_body
    )
...
Enviado desde mi oficina móvil BlackBerry® de Telcel"""
    ...
    assert (
        ("Blah", "Enviado desde mi oficina móvil BlackBerry® de Telcel")
    ) == bruteforce.extract_signature(msg_body)
...
John Doe"""
    assert (("Thanks,\n\nBlah", "regards\n\nJohn Doe")) == bruteforce.extract_signature(
        msg_body
    )
...
def test_get_signature_candidate():
    ...
    with patch.object(bruteforce, "SIGNATURE_MAX_LINES", 2):
        ...
        assert ["signature1", "", "signature2"] == bruteforce.get_signature_candidate(
            lines
        )
...
def test_mark_candidate_indexes():
    with patch.object(bruteforce, "TOO_LONG_SIGNATURE_LINE", 3):
        ...
        assert "clc" == bruteforce._mark_candidate_indexes(
            ["BR,  ", "long", "Bob"], [0, 1, 2]
        )

        # only candidate lines are marked
        # if line has only dashes it's a candidate line
        assert "ccdc" == bruteforce._mark_candidate_indexes(
            ["-", "long", "-", "- i", "Bob"], [0, 2, 3, 4]
        )


def test_process_marked_candidate_indexes():
    assert [2, 13, 15] == bruteforce._process_marked_candidate_indexes(
        [2, 13, 15], "dcc"
    )

    assert [15] == bruteforce._process_marked_candidate_indexes([2, 13, 15], "ddc")

    assert [13, 15] == bruteforce._process_marked_candidate_indexes([13, 15], "cc")

    assert [15] == bruteforce._process_marked_candidate_indexes([15], "lc")

    assert [15] == bruteforce._process_marked_candidate_indexes([13, 15], "ld")
