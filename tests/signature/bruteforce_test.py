...
Enviado desde mi oficina mÃ³vil BlackBerryÂ® de Telcel"""
    ...
    eq_(("Blah", "Enviado desde mi oficina mÃ³vil BlackBerryÂ® de Telcel"), bruteforce.extract_signature(msg_body))
...
def test_mark_candidate_indexes():
    with patch.object(bruteforce, "TOO_LONG_SIGNATURE_LINE", 3):
        ...
        eq_("clc", bruteforce._mark_candidate_indexes(["BR,  ", "long", "Bob"], [0, 1, 2]))

        # only candidate lines are marked
        # if line has only dashes it's a candidate line
        eq_("ccdc", bruteforce._mark_candidate_indexes(["-", "long", "-", "- i", "Bob"], [0, 2, 3, 4]))
