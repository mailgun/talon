...
SPLITTER_PATTERNS = [
    ...
    re.compile(
        r"\S{3,10}, \d\d? \S{3,10} 20\d\d,? \d\d?:\d\d(:\d\d)?"
        r"( \S+){3,6}@\S+:"
    ),
...
def extract_from_html_tree(html_tree):
    ...
    if not lines_were_deleted and not cut_quotations:
        # No changes made by either HTML-specific or plain-text algorithms.
        # html_tree_copy here is a deepcopy of the original html_tree input to this function,
        # because cut_quotations was False (so html_tree_param was not modified before deepcopy)
        # and lines_were_deleted was False.
        if _readable_text_empty(html_tree_copy):
            return None
        # If original was not empty and we are not stripping it,
        # we fall through to the stringify part at the end of the function,
        # which will operate on html_tree_copy (the representation of original content).

    elif lines_were_deleted:  # This 'elif' ensures this block is skipped if the above 'if' was true and didn't return None
    ...
    # Use encoding='unicode' to get a Python unicode string directly from lxml
    s = html.tostring(html_tree_copy, encoding="unicode", method="html")
    if not s:
        return None

    # No need to decode if html.tostring returned a unicode string
    return s
